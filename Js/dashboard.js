...
function initDashboardPage() {
    ...
    // Search Input Event (Using .off() to prevent duplicate events if visited twice)
    ...
    // Dropdown bahar click karne par band ho
...
// Baki ka purana code (initMap, initChart, loadDashboardData, etc.) yahan niche waise hi rahega...

// Map Initialization
function initMap() {
    if (!document.getElementById("geofenceMap")) return; // Guard
...
function initChart() {
    ...
    if (sourceChart) sourceChart.destroy(); // Purana chart memory se clear karein
...
// Pagination Logic (Globally available for HTML onclicks)
